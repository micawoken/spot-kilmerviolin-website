...
export function auth_check(
    ...
    required_perms: (keyof RoleProfile)[],
    fail_closed: boolean = true,
    mode: "default" | "selfmgmt" | "enroll" = "default"
): Response | null {
 ...
 */
function _checkInactiveCredential(
    request: Request,
    identity: Identity,
    required_perms: (keyof RoleProfile)[],
    mode: "default" | "selfmgmt" | "enroll"
): Response | null {
    ...
    if (!identity.enrollable && mode !== "selfmgmt") {
        ...
        if (required_perms.length > 0) {
            ...
            return constructResponse(
                ...
                null,
                403,
                "Forbidden - credential lacking active authorization information"
            )
        }
    ...
    if (mode !== "enroll") {
        return constructResponse(
            ...
            null,
            401,
            "Unauthorized - credential lacks authorization" +
                (env.API_USER_SELFENROLL ? "; perform self-enrollment" : "")
        )
    } else {
