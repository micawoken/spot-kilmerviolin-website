...
export async function getKey(key: string, properties?: Partial<KVNamespaceGetOptions<undefined>>): Promise<any> {
    ...
    if ((value.metadata as KVMetadata).v === 1) {
        ...
        if ((value.metadata as KVMetadata).h) {
            // has data
            const output =
                (value.metadata as KVMetadata)?.f === "json" ? JSON.parse(value.value as string) : value.value
            return output
        } else {
            // no data
            return undefined
    ...
    if (!records) {
        ...
        return key_data.reduce(
            (acc, k) => {
                ...
                if (k.metadata.v === 1) {
                    acc[k.name] =
                        (k.metadata as KVMetadata)?.f === "json"
                            ? JSON.parse(k.metadata.value as string)
                            : k.metadata.value
                    return acc
