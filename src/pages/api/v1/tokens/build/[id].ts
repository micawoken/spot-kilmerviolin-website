...
import { buildTokenExists, revokeBuildToken } from "../../../../../lib/api/tokens"
...
export const DELETE: APIRoute = async (context): Promise<Response> => {
    ...
    try {
        if (!(await buildTokenExists(state_id))) {
            return constructResponse(request, null, 404)
        }
        await revokeBuildToken(state_id, Date.now())
        return constructResponse(request, null, 204)
    } catch (error) {
        return constructResponseErrorHook(request, error, 500, "Failed to revoke build token")
    }
}
