...
export const PUT: APIRoute = async (context): Promise<Response> => {
    return constructResponse(context.request, null, 405, "Method not allowed: use PATCH /api/v1/identity (for identity-centric updates) or PUT /api/v1/contributors/[id] (for contributor-centric updates)", {
...
async function _parseAdmin(context: APIContext, request: Request, payload: {admin: unknown}): Promise<string[] | Response> {
    ...
    if (typeof payload.admin !== "object" || payload.admin === null) {
        return constructResponse(request, null, 400, "Bad request: admin field if specified must be an object; previous transactions may have succeeded")
    }
    if ("elevate" in payload.admin) {
        if (!Array.isArray(payload.admin.elevate) || !payload.admin.elevate.every((email: any) => typeof email === "string")) {
            return constructResponse(request, null, 400, "Bad request: elevate field if specified must be a list of strings (emails); previous transactions may have succeeded")
        }
        if (payload.admin.elevate.length > 5) {
            return constructResponse(request, null, 400, "Bad request: transaction exceeds max 5 users per admin.elevate; previous transactions may have succeeded")
        }
    ...
    if ("demote" in payload.admin) {
        if (!Array.isArray(payload.admin.demote) || !payload.admin.demote.every((email: any) => typeof email === "string")) {
            return constructResponse(request, null, 400, "Bad request: demote field if specified must be a list of strings (emails); previous transactions may have succeeded")
        }
        if (payload.admin.demote.length > 5) {
            return constructResponse(request, null, 400, "Bad request: transaction exceeds max 5 users per admin.demote; previous transactions may have succeeded")
        }
...
async function _parseActive(context: APIContext, request: Request, payload: {active: unknown}): Promise<string[] | Response> {
    let errors: string[] = []
    if (typeof payload.active !== "object" || payload.active === null) {
        return constructResponse(request, null, 400, "Bad request: active field if specified must be an object; previous transactions may have succeeded")
    }
    if ("activate" in payload.active) {
        if (!Array.isArray(payload.active.activate) || !payload.active.activate.every((email: any) => typeof email === "string")) {
            return constructResponse(request, null, 400, "Bad request: activate field if specified must be a list of strings (emails); previous transactions may have succeeded")
        }
        if (payload.active.activate.length > 5) {
            return constructResponse(request, null, 400, "Bad request: transaction exceeds max 5 users per active.activate; previous transactions may have succeeded")
        }
    ...
    if ("deactivate" in payload.active) {
        if (!Array.isArray(payload.active.deactivate) || !payload.active.deactivate.every((email: any) => typeof email === "string")) {
            return constructResponse(request, null, 400, "Bad request: deactivate field if specified must be a list of strings (emails); previous transactions may have succeeded")
        }
        if (payload.active.deactivate.length > 5) {
            return constructResponse(request, null, 400, "Bad request: transaction exceeds max 5 users per active.deactivate; previous transactions may have succeeded")
        }
...
async function _parseRoles(context: APIContext, request: Request, payload: {roles: unknown}): Promise<string[] | Response> {
    let errors: string[] = []
    if (typeof payload.roles !== "object" || payload.roles === null) {
        return constructResponse(request, null, 400, "Bad request: roles field if specified must be an object; previous transactions may have succeeded")
    }
    if ("add" in payload.roles) {
        if (typeof payload.roles.add !== "object" || payload.roles.add === null) {
            return constructResponse(request, null, 400, "Bad request: add field if specified must be an object; previous transactions may have succeeded")
        }
        // length check
        if (Object.keys(payload.roles.add).length > 5) {
            return constructResponse(request, null, 400, "Bad request: transaction exceeds max 5 users per roles.add; previous transactions may have succeeded")
        }
        for (const [email, roles_add] of Object.entries(payload.roles.add)) {
            if (!Array.isArray(roles_add) || !roles_add.every((role: any) => role in roles)) {
                return constructResponse(request, null, 400, `Bad request: roles to add for ${email} must be valid roles`)
            }
            const id = await emailToId(email)
            if (id === null) {
            ...
            if (roles_add.length > 5) {
                return constructResponse(request, null, 400, `Bad request: transaction exceeds max 5 roles per user for roles.add; previous transactions may have succeeded`)
            }
    ...
    if ("remove" in payload.roles) {
        if (typeof payload.roles.remove !== "object" || payload.roles.remove === null) {
            return constructResponse(request, null, 400, "Bad request: remove field if specified must be an object; previous transactions may have succeeded")
        }
        if (Object.keys(payload.roles.remove).length > 5) {
            return constructResponse(request, null, 400, "Bad request: transaction exceeds max 5 users per roles.remove; previous transactions may have succeeded")
        }
        for (const [email, roles_remove] of Object.entries(payload.roles.remove)) {
            ...
            if (!Array.isArray(roles_remove) || !roles_remove.every((role: any) => role in roles)) {
                return constructResponse(request, null, 400, `Bad request: roles to remove for ${email} must be valid roles; previous transactions may have succeeded`)
            }
            if (roles_remove.length > 5) {
                return constructResponse(request, null, 400, `Bad request: transaction exceeds max 5 roles per user for roles.remove; previous transactions may have succeeded`)
            }
...
async function _parseIdEmail(context: APIContext, request: Request, payload: {identity_email: unknown}): Promise<string[] | Response> {
    let errors: string[] = []
    if (typeof payload.identity_email !== "object" || payload.identity_email === null) {
        return constructResponse(request, null, 400, "Bad request: identity_email field if specified must be an object; previous transactions may have succeeded")
    }
    if (Object.keys(payload.identity_email).length > 5) {
        return constructResponse(request, null, 400, "Bad request: transaction exceeds max 5 users per identity_email; previous transactions may have succeeded")
    }
