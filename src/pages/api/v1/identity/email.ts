...
export const PATCH: APIRoute = async (context): Promise<Response> => {
    ...
    if (typeof api_request.payload[0] !== "object" || api_request.payload[0] === null) {
        return constructResponse(request, null, 400, "Bad request: payload must be a JSON object mapping current emails to new emails")
    }
    ...
    for (const [old_email, new_email] of Object.entries(email_map)) {
        if (typeof new_email !== "string" || new_email.trim() === "" || !new_email.includes("@")) {
            return constructResponse(request, null, 400, `Bad request: new email for ${old_email} must be a valid email string`)
        }
        ...
        if (isFallbackEmail(new_email)) {
            return constructResponse(request, null, 400, `Bad request: cannot set the identity email for ${old_email} to a reserved fallback address`)
        }
    }
    const errors: string[] = []
    ...
    try {
        ...
        const response = constructResponseErrorHook(request, error, 500, "Failed to change identity email; previous transactions may have succeeded")
