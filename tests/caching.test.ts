...
describe("Cache TTL policy", () => {
    it("uses the short TTL when long is false", async () => {
        ...
        const response = await _getCache("test_store", "https://cache.local/cache/short_ttl_key")
    ...
    it("uses the long TTL when long is true", async () => {
        ...
        const response = await _getCache("test_store", "https://cache.local/cache/long_ttl_key")
        expect(response).toBeDefined()
